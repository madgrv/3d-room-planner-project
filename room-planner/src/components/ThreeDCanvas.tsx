...
interface SceneContentProps {
  onRightClick: (e: MouseEvent, itemId: string | null, roomElement: RoomElementType) => void;
  backgroundColor: string;
  theme: string;
  snapEnabled: boolean;
}

const SceneContent = ({ onRightClick, backgroundColor, theme, snapEnabled }: SceneContentProps) => {
  ...
  const clearFurnitureSelection = useFurnitureStore((state) => state.clearSelection);
  ...
  const handlePointerDown = useCallback((e: MouseEvent) => {
    ...
    console.log('Raycasting result:', clickedItemId ? `Hit item: ${clickedItemId}` : 'No item hit');
  ...
  }, [camera, gl, mouse, onRightClick, raycaster, scene.children, setSelectedElement, clearFurnitureSelection]);
...
export function ThreeDCanvas({ snapEnabled = false }: ThreeDCanvasProps) {
  ...
  const globalIsDragging = useDragStore((state: { isDragging: boolean }) => state.isDragging);
  ...
  const handleRightClick = useCallback((e: MouseEvent, itemId: string | null, roomElement: RoomElementType) => {
